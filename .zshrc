...
autovenv() {
    usage() {
        ...
        printf "    This function is normally invoked automatically via precmd hook\n"
...
star() {
    ...
    local row
    if $reverse; then
        for ((row = n; row >= startfrom; row--)); do
            if [[ "$align" == "right" ]]; then
        ...
        for ((row = startfrom; row <= n; row++)); do
            if [[ "$align" == "right" ]]; then
                printf "%${n}s\n" "$(printf "%${row}s" | tr ' ' "$character")"
            else
                printf "%${row}s\n" | tr ' ' "$character"
